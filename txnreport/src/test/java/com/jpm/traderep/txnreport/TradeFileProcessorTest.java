...
public class TradeFileProcessorTest {
	...
	public void populateMapTest20160104() {
		...
		BigDecimal usdSettledAmountIncoming = tfp.getTxnDataMap().get(settleDate).get(entity).getUsdSettledAmountIncoming();
		BigDecimal usdSettledAmountOutgoing = tfp.getTxnDataMap().get(settleDate).get(entity).getUsdSettledAmountOutgoing();
		assertEquals(actualSettlementDate, new Integer(settleDate));
		assertEquals(
				BigDecimal.ZERO.setScale(2, BigDecimal.ROUND_HALF_UP),usdSettledAmountIncoming.setScale(2, BigDecimal.ROUND_HALF_UP));
		assertEquals(usdSettledAmountOutgoing.setScale(2, BigDecimal.ROUND_HALF_UP),
				new BigDecimal(2155.00).setScale(2, BigDecimal.ROUND_HALF_UP));
		
//		for (Integer i: tfp.getTxnDataMap().keySet()) {
//			System.out.println(i + " " + tfp.getTxnDataMap().get(i));
//		}
	...
	public void populateMapTest20160107() {
		...
		BigDecimal usdSettledAmountOutgoing = tfp.getTxnDataMap().get(settleDate).get(entity).getUsdSettledAmountOutgoing();
		assertEquals(actualSettlementDate, new Integer(settleDate));
		assertEquals(usdSettledAmountIncoming.setScale(2, BigDecimal.ROUND_HALF_UP), new BigDecimal(14899.50).setScale(2, BigDecimal.ROUND_HALF_UP));
		assertEquals(usdSettledAmountOutgoing.setScale(2, BigDecimal.ROUND_HALF_UP),BigDecimal.ZERO.setScale(2, BigDecimal.ROUND_HALF_UP));
	...
	public void populateMapTest20160110() {
		...
		assertEquals(actualSettlementDate, new Integer(settleDate));
		assertEquals(usdSettledAmountOutgoing.setScale(2, BigDecimal.ROUND_HALF_UP), BigDecimal.ZERO.setScale(2, BigDecimal.ROUND_HALF_UP));
		assertEquals(usdSettledAmountIncoming.setScale(2, BigDecimal.ROUND_HALF_UP),
				new BigDecimal(14237.30).setScale(2, BigDecimal.ROUND_HALF_UP));
